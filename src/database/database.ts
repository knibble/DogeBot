...
const sequelize = new Sequelize({
    ...
    logging: config.env.type == Environment.DEVELOPMENT
});

sequelize
    .authenticate()
    .then(() => {
        logger.info(`Authenticated into database (${config.env.database}).`)
    })
    .catch(() => {
...
function synchronize(force: boolean = false) {
    try {
        ...
        logger.info(`${force ? 'Created' : 'Initialized'} ${_models.length} model${_models.length > 1 ? 's' : ''}.`)
        logger.debug(`Model${_models.length > 1 ? 's' : ''}: ${_models.join(', ')}.`)
    } catch (error) {
